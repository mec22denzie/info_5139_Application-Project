  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      for (const item of verifiedItems) {
        // Find the already accepted order for this buyer and product
        const orderQ = query(
        ...
        if (orderSnap.empty) {
          return showAlert("Error", `${item.name} is not yet accepted by the donor.`);
        }

        const orderDoc = orderSnap.docs[0];

        // Mark order as completed _m
        ...
        // Mark product as completed / unavailable _m
        if (item.productId) {
          await updateDoc(doc(firestore, "products", item.productId), {
            status: "completed",
            isAvailable: false,
            updatedAt: new Date().toISOString(),
          });
      ...
      // Send notifications to donors whose items were ordered _m
      const donorIds = [...new Set(verifiedItems.filter((it) => it.donorId).map((it) => it.donorId))];

      for (const donorId of donorIds) {
        const donorItems = verifiedItems.filter((it) => it.donorId === donorId);
